...
public class RedHenCounterfactualityCycle extends CounterfactualityCycle {
	...
	public RedHenCounterfactualityCycle(PlotDirectedSparseGraph originalGraph, MoodMapper originalMood) {
		super("agent_folktale_animal", new String[] {"hen", "cow", "dog", "pig"}, originalGraph, originalMood, "hen");
...
//		setEndCycle(10);
	...
	public List<Personality[]> createPlotSpace(Personality[] personalitySpace, int characters) {
		...
		List<Personality[]> allPersonalityCombinations = new LinkedList<>();
		if(!this.allCharacters) {
			this.log("right choice");
			...
			this.log("I start creating the plot space");
			List<int[]> values = this.allCombinations(characters, personalitySpace.length, this.allCharacters);
			this.log("I have calculated all Combinations");
			this.log("Now I start the for loop");
	...
	public PlotModel<?> getPlotModel(List<LauncherAgent> agents) {
		FarmModel model = new FarmModel(new ArrayList<LauncherAgent>(), this.getHappeningDirector(agents));
