# inBloom
InBloom is a system for generating narratives, which is using an extended version of the [Jason multi-agent framework](https://github.com/cartisan/jason/tree/o3a). It's an ongoing research project into computational storytelling and
...
The name inBloom is short for ‘inside Bloom’; a homage to Joyce’s ingenious character construction as exemplified by Ulysses' Leopold Bloom.
...
1. Import the inBloom project in Eclipse
...
To implement your own story and explore it using inBloom you need to extend several classes and implement custom agent reasoning. An example story can be found in the `inBloom.stories.little_red_hen` package.<br>
<i>Note: It is advisable to first get familiar with the basics of Jason programming, using for instance this [getting started guide](http://jason.sourceforge.net/mini-tutorial/getting-started/#_an_example_with_environment) or [this tutorial](http://jason.sourceforge.net/Jason.pdf). You can play around with the tutorials in the `inBloom.tutorial` package.</i>

1. Implement a your custom AgentSpeak reasoning code in a custom `src/asl/agentXYZ.asl`
1. Create a new sub-package `inBloom.stories.XYZ` under `src/java` for your story, all your Java classes should be located here
